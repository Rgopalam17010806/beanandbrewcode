...
class TableBookingForm(FlaskForm):
    firstname = StringField('First Name', validators=[DataRequired()])
    lastname = StringField('Last Name', validators=[DataRequired()])
    location = SelectField('Location', choices=[('Leeds', 'Leeds'), ('Harrowgate', 'Harrowgate'),
                                                ('Knaresborough castle', 'Knaresborough castle')])
    members = SelectField('Members', choices=[(str(i), str(i)) for i in range(1, 11)])
    timings = SelectField('Timings', choices=[('Morning (8:00 to 11:00)', 'Morning (8:00 to 11:00)'),
                                              ('Afternoon (11:00 to 17:00)', 'Afternoon (11:00 to 17:00)'),
                                              ('Evening (17:00 to 22:00)', 'Evening (17:00 to 22:00)')])
...
class OnlineBakingLessonBookingForm(FlaskForm):
    email = StringField('Email', validators=[DataRequired()])
    course_name = SelectField('Category', choices=[('How to bake Butter Croissants', 'How to bake Butter Croissants'),
                                                   ('How to Bake Chocolate Cake', 'How to Chocolate Cake'),
